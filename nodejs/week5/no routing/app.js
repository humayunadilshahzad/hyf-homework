...
app.get('/', (req, res) => { res.send('Bismillah HYF Students'); });

app.get('/studentsList', (req, res) => {
    let studentsList = hyf_students.getStudentsList();
    if (studentsList) {
        res.status(201);
        ...
        res.status(404);
        res.send('No student exists....');
    }

});

app.get('/listByclassId', (req, res) => {
    let listByClass = hyf_students.getStudentsFilterByClass(req.query.classId);
    if (listByClass) {
        res.status(201);
        res.send(listByClass);
    }
    else {
        res.status(404);
        res.send(`class ${req.query.classId} do not exist....`);
...
app.get('/findStudent', (req, res) => {
    let foundStuddent = hyf_students.StudentSearchByName(req.query.name);
    if (foundStuddent) {
        res.status(201);
        res.send(foundStuddent);
    }
    else {
        res.status(404);
...
app.post('/addNewStudent', (req, res) => {
    let studentToAdd = hyf_students.addNewStudent(req.body);
    if (studentToAdd) {
        res.status(201);
        res.send(req.body.name + ' student added');
    }
    else {
        res.status(404);
...
app.put('/editStudent', (req, res) => {
    let editedStudent = hyf_students.updateStudentRecord(req.body);
    if (editedStudent) {
        res.status(201);
        res.send(req.body.name + ' record updated...');
    }
    else {
        res.status(404);
        res.send(req.body.name + ' do not exists....');
    }

});

app.delete('/deleteStudent', (req, res) => {
    let deletedStudent = hyf_students.deleteStudent(req.body);
    if (deletedStudent) {
        res.status(201);
        res.send(`${req.body.name} is deleted`);
    }
    else {
        res.status(404);
...
app.listen(8002, () => console.log('up at 8002'));
/* const server = http.createServer((req, res) => {
    ...
    if (url === '/getList' && req.method === 'GET') {
        if (hyf_students.getStudentsList) {
            res.statusCode = 201;
        ...
        if (hyf_students.getStudentsFilterByClass('09')) {
            res.statusCode = 201;
        ...
        if (hyf_students.StudentSearchByName('Humayun Adil')) {
            res.statusCode = 201;
        ...
        const isAdded = hyf_students.addNewStudent(newStudent);
            if(isAdded){
                res.statusCode=200;
                ...
                res.statusCode=401;
                res.end('studdent already exists');
            }
